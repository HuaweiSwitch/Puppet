	  ...
	  class TrunkApi < ApiBase
		...
		def get_trunk()			  
		  ...
		  trunk_elements.element_children.each do |trunk_elem|
			...
			if trunk_untagged_vlan
			  property_hash[:untagged_vlan] = trunk_untagged_vlan
			end
		...
		def set_trunk(resource)	
		  ...
		  if resource[:mode] == :trunk
			...
			# delete tagged_vlans
			get_trunk_xml = '<rpc><get><filter type="subtree"><ethernet xmlns="http://www.huawei.com/netconf/vrp" content-version="1.0" format-version="1.0"><ethernetIfs><ethernetIf><ifName>' + "#{resource[:name]}" + '</ifName><l2Attribute><linkType></linkType><pvid></pvid><trunkVlans></trunkVlans></l2Attribute></ethernetIf></ethernetIfs></ethernet></ifm></filter></get></rpc>'
			...
			delete_trunk_xml = '<rpc><edit-config><target><running/></target><default-operation>merge</default-operation><error-option>rollback-on-error</error-option><config><ethernet xmlns="http://www.huawei.com/netconf/vrp" content-version="1.0" format-version="1.0"><ethernetIfs><ethernetIf operation="merge"><ifName>' + "#{resource[:name]}" + '</ifName><l2Attribute><linkType>trunk</linkType><pvid>1</pvid><trunkVlans>' + "#{aa}" + ':'+"#{dd}" +'</trunkVlans></l2Attribute></ethernetIf></ethernetIfs></ethernet></config></edit-config></rpc>'
			...
			set_trunk_xml = '<rpc><edit-config><target><running/></target><default-operation>merge</default-operation><error-option>rollback-on-error</error-option><config><ethernet xmlns="http://www.huawei.com/netconf/vrp" content-version="1.0" format-version="1.0"><ethernetIfs><ethernetIf operation="merge"><ifName>' + "#{resource[:name]}" + '</ifName><l2Attribute><linkType>trunk</linkType><pvid>' + "#{resource[:untagged_vlan]}" + '</pvid><trunkVlans>' + "#{aa}" + ':'+"#{aa}" +'</trunkVlans></l2Attribute></ethernetIf></ethernetIfs></ethernet></config></edit-config></rpc>'
		  end
